...
实现者不能自己签发独立 QA
高风险任务默认暂停，直到接入可审计确认
反馈默认先预览并尽力脱敏，再由用户决定是否提交
```

它不是“再写一段更严厉的 prompt”，而是把提示词、项目规则、Issue/PR 模板、验证脚本、CI 门禁和独立 QA 证据串成一条可审计交付链。
...
| 测试只剩一句“应该没问题” | PR 必须记录命令、退出码、commitSha 和证据链接；真实测试执行依赖目标仓库 CI 或用户配置 |
| 实现者自己说自己通过 | L2/L3 必须记录未参与实现的上下文 QA 证据；强防伪仍依赖外部可审计事件 |
| 高风险改动被自动化直接推进 | L3 涉及数据、权限、密钥、真实 Provider、生产环境时默认 fail-closed，必须接入可审计确认 |
| 用户反馈散在聊天里，无法迭代产品 | Beta feedback Issue 模板 + 本地 best-effort 脱敏反馈导出 |

## 它到底做了什么

Agents-Team 给 Codex 长任务加上一条 GitHub-first 的交付安全带：
...
Agents-Team 不是给每一次小改都套流程。以下场景通常不值得引入：

| 场景 | 建议 |
| --- | --- |
| 改一行文案、一次性脚本、纯本地实验 | 直接让 Codex 处理，不需要完整 Issue/PR/QA 链 |
| 没有 Git 仓库、没有 PR、没有 CI | 先建立基础工程流程，或只借鉴 `AGENTS.md` / PR 模板 |
| 无法接受仓库模板或 GitHub Actions 文件变更 | 只使用 dry-run 预览，不 apply |
| 希望自动上传日志、源码或遥测 | 不适合；Agents-Team 默认不做静默上报 |
| 已有成熟 Jira/Linear/GitLab/内部审计链 | 先评估是否只需要 validator 或 PR 证据格式 |

最适合的第一批用户是：已经频繁让 Codex 处理跨文件任务，并愿意用 GitHub Issue、PR、CI 和独立 QA 来减少返工的人。
...
Agents-Team 现在适合 **公开 Beta / source-available preview** 试用，当前许可证不是开源许可证；不要把它第一天就扔进不可回滚的生产仓库。
...
当前阶段不追求大规模安装数，优先验证它是否真的降低长任务交付风险。

| 目标 | 判断标准 |
| --- | --- |
| 5-10 个重度 AI coding 用户试用 | 至少有人完整跑通 L2 Goal、PR、测试证据和独立 QA |
| 真实发现问题 | 能拦住或暴露边界扩大、证据缺失、commitSha 不一致、QA 依据不足等问题 |
| 摩擦可接受 | 用户愿意在下一个跨文件任务继续使用，而不是只觉得“表格太多” |
| 失败可复盘 | 卡点能沉淀为 GitHub Issue，而不是散在聊天记录里 |
...
| 反馈先预览 | 反馈 Issue 默认只生成草稿，并对常见敏感信息 best-effort 脱敏；确认后才允许提交到 GitHub |
| 高风险 fail-closed | L3 触及数据、权限、密钥、真实 Provider 或生产环境时必须暂停；没有可审计批准事件就不得通过 Gate |
| 独立 QA 留证据 | 没有真正独立上下文，就不能声明独立验收通过；当前 Gate 只能机械检查证据完整性和一致性 |
...
它真正做的，是把人和 Codex 之间容易丢失的约定，变成项目中看得见、跑得动、可复盘的工程证据链。当前版本能机械检查合同和证据的一部分，但不能替代真实 CI、分支保护、用户批准或人工判断。
