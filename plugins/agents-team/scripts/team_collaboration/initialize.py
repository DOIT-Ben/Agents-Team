...
def initialize_project(root: Path, plugin_root: Path, *, apply: bool = False, allow_dirty: bool = False) -> dict[str, Any]:
    ...
    for template, target in TEMPLATE_TARGETS.items():
        ...
        if destination.exists() and destination.read_bytes() != data:
            marker = b"Generated by agents-team"
            if marker not in destination.read_bytes():
                raise InitializationError(f"existing unmanaged target would be overwritten: {target}")
