...
### P1：仓库没有用 CI 约束自身，协作机制没有自托管验证

仓库会向下游项目生成 `.github/workflows/collaboration-gate.yml`，但 Agents-Team 自身根目录没有 `.github/workflows/`。远程 PR #4 查询结果同样为：

```text
no checks reported on the branch
```

因此当前仓库的提交和 PR 不会自动运行：
...
这使“协作机制要求下游具备强门禁”与“机制自身没有门禁”形成自托管缺口，也是测试接口漂移能够进入默认分支的重要原因。

建议至少增加 Linux 与 Windows 两套 CI，并将测试、分发验证、可信门禁攻击用例设为默认分支必需检查。发布版本必须来自通过这些检查的固定提交，而不是本地生成后直接发布。
...
1. 合并重复验证实现。
2. 修复 9 项失败/错误测试。
3. 为仓库自身增加 Linux 与 Windows CI，并配置默认分支必需检查。
